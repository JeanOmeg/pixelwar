...
import type { ClassType, UnitType } from '../config'
...
export class LevelBase extends ex.Scene {
  ...
  parse(levelData: LevelData) {
    Resources.LevelMusic2.loop = true
    void Resources.LevelMusic2.play()
    ...
    if (mode === 'p1VsCpuButton') {
      ...
      this.players = [
        ...
        new HumanPlayer(`${levelData.players[1]} B`, this.engine, this.selectionManager, this.uiManager, board),
      ]
    } else {
      this.players = [
        new ComputerPlayer(`${levelData.players[0]} A`, this.selectionManager, board),
