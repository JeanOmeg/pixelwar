...
import type { UnitConfig, UnitType } from './config';
import { SCALE, UNIT_CONFIG } from './config'
...
export class Unit extends ex.Actor {
  ...
  override onPostUpdate() {
    ...
    if (this.health <= 0) {
      ...
      void this.actions.runAction(deathAnim).toPromise()
      void Resources.ExplosionSound.play()
      void ex.Util.delay(600)
