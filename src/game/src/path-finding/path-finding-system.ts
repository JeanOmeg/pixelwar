...
export class PathFinder {
  ...
  getXOrYByNameCell(name: string, direction: 'x' | 'y'): string {
    if (direction === 'x') {
      return name.split(' ')[1]!.split(')')[0] as string
    } else {
      return name.split(',')[0]!.split('(')[1] as string
    }
  }
  ...
  findPath(start: PathNodeComponent, end: PathNodeComponent, mask: number, range?: PathNodeComponent[]): PathNodeComponent[] {
    const nodes = this.query.getEntities().map(n => n.get(PathNodeComponent))
    nodes.forEach(node => {
      node.gScore = 0
      node.hScore = 0
    ...
    while (openNodes.length > 0) {
      ...
      const current = priorityNodes[0] as PathNodeComponent
